...
window.onload = function init(){
    ...
    //import shaders
    gl.program = initShaders(gl, "Shaders/vshaderw6p2.glsl", "Shaders/fshaderw6p2.glsl");
    ...
    // create mipmap for the checkerboard texture:
    gl.generateMipmap(gl.TEXTURE_2D);
    var WrappingMode;
    // create event listeners for the wrap&filter mode selection menus:
    const WrappingMenu = document.getElementById("WrappingMenu");
    const filterMenu = document.getElementById("filterMenu");
    
    WrappingMenu.addEventListener("change", function (event) {
        WrappingMode = null;
        if (WrappingMenu.value == "repeat") {
            WrappingMode = gl.REPEAT;
        }
        else {
            WrappingMode = gl.CLAMP_TO_EDGE;
        }
    ...
    filterMenu.addEventListener("change",function(event){
        if(filterMenu.value=="nearest"){
            gl.texParameteri(gl.TEXTURE_2D,gl.TEXTURE_MAG_FILTER,gl.NEAREST);
            gl.texParameteri(gl.TEXTURE_2D,gl.TEXTURE_MIN_FILTER,gl.NEAREST);
        }else if(filterMenu.value == "linear"){
